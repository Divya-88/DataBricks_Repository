...
# drop table lakehousecat.deltadb.drugstbl;
# drop table lakehousecat.deltadb.drugstbl_merge;
# drop table lakehousecat.deltadb.drugstbl_partitioned;
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM parquet.`/Volumes/lakehousecat/deltadb/datalake/targetdirparquet/`
# MAGIC LIMIT 10;
...
# MAGIC %sql
# MAGIC DESC HISTORY lakehousecat.deltadb.drugstbl -- here DESC refers to describe, if given inside orderby --> descending
...
# MAGIC %sql 
# MAGIC select * from drugstbl version as of 2
# MAGIC where uniqueid=163740;
...
df.where('uniqueid=163740').show()

# COMMAND ----------

from delta.tables import DeltaTable
deltaTable = DeltaTable.forPath(spark, "/Volumes/lakehousecat/deltadb/datalake/targetdir") #Loads a Delta table from a folder path
# 1. delta does not delete files row by row
# 2. it rewrites parquet files without deleted rows
...
# MAGIC select * from drugstbl where rating<=8;
...
# MAGIC -- select * from drugstbl_merge;
